...
const Header: React.FC<HeaderProps> = ({ title, onBack, rightAction, avatarUrl, onTitleClick }) => {
  ...
  return (
    <div className="sticky top-0 z-40 h-16 w-full p-3 flex items-center justify-between pointer-events-none">
      {hasLeftContent ? (
        <div className="p-1 bg-light-bg-secondary/80 dark:bg-dark-bg-secondary/80 rounded-full backdrop-blur-lg shadow-md pointer-events-auto flex items-center space-x-2">
          {onBack && (
