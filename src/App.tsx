...
const App: React.FC = () => {
  ...
  const hydratedConversations: HydratedConversation[] = useMemo(() => {
    ...
    // Fix: Explicitly type the return value of the map function to HydratedConversation.
    // This resolves a TypeScript error where the spread operator on a union type (`convo`)
    // caused the resulting object's `type` property to be inferred as a generic `string`
    // instead of the more specific `'private' | 'group'`.
    return conversations.map((convo): HydratedConversation => {
      // Treat conversations without a 'type' as private for backward compatibility.
      if (convo.type === 'private' || !convo.type) {
        const participantId = convo.participants.find(p => p !== currentUser.id);
        const participant = users.find(u => u.id === participantId);
        // Ensure the type is set for the component's filtering logic.
        return { ...convo, type: 'private', participant };
      }
      return { ...convo }; // For group chats
  ...
  const renderScreen = () => {
    ...
    switch (activeScreen) {
      case 'adventures': return <AdventuresScreen adventures={hydratedAdventures.filter(p => !p.author.isPrivate || (currentUser?.following?.includes(p.author.id)) || p.author.id === currentUser?.id)} stories={hydratedStories} currentUser={currentUser || guestUser} onSelectAdventure={setSelectedAdventure} onSendMessage={handleStartPrivateConversation} onToggleInterest={handleToggleInterest} onSelectStories={(stories: HydratedStory[]) => { setViewingStories(stories); const firstStory = stories[0]; if(firstStory){ const newTimestamps = {...viewedStoryTimestamps, [firstStory.author.id]: new Date().toISOString()}; setViewedStoryTimestamps(newTimestamps); localStorage.setItem('viewedStoryTimestamps', JSON.stringify(newTimestamps)); } }} onAddStory={() => setIsAddStoryModalOpen(true)} onNavigateToNotifications={() => pushScreen('notifications')} hasUnreadNotifications={hasUnreadNotifications} onNavigateToChat={() => pushScreen('chat')} onViewProfile={(user: User) => { setViewingUser(user); pushScreen('userProfile'); }} onRepostToggle={handleToggleRepost} onSaveToggle={handleToggleSave} onShareAdventure={handleShareAdventure} onToggleCompleted={handleToggleCompleted} isGuest={isGuest} onGuestAction={handleGuestAction} onNavigateToSearch={() => resetToScreen('search')} onNavigateToProfile={() => resetToScreen('profile')} onViewLocationOnMap={(adv: HydratedAdventure) => { setMapAdventuresToShow([adv]); resetToScreen('map'); }} onDeleteAdventure={handleDeleteAdventure} onEditAdventure={handleStartEditAdventure} viewedStoryTimestamps={viewedStoryTimestamps} onJoinGroupChat={handleJoinAdventureChat} />;
